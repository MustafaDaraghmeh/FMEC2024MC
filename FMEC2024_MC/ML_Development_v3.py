...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s',
                    handlers=[
                        logging.FileHandler("ML_Development_v3.log", mode='w'),
...
def plot_model(classification_type, X_train, y_train, X_test, y_test, model, figs_path, model_index, title):
    ...
    # To copy and wrap the original model
    if model_index in ['XGB', 'CB']:
...
def main():
    ...
    logging.info(f'Prepare the multi-class classification dataset (Drone Types)')
    logging.info(f'Remove the Noise Class to keep only the drone classes')
    multi_class_df = dataset.copy()
    multi_class_df = multi_class_df.drop(multi_class_df[multi_class_df['Class'] == 'Noise'].index)
    multi_class_df.reset_index(drop=True, inplace=True)
    # Run the classification pipeline in the multi-class dataset
    run_classification_pipeline(dataset=multi_class_df,
                                      experiment_name='Multi-class - Drone Type',
                                      m_select=10,
